...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # this is a common practice in GPT models to share weights between the token embedding and the output layer
        # it reduces the number of parameters and can help with generalization
    ...
    def from_pretrained(cls, model_type):
        ...
        assert len(sd_keys_hf) == len(sd_keys), f"mismatched keys: {len(sd_keys_hf)} != {len(sd_keys)}, keys not matching: {set(sd_keys_hf) - set(sd_keys)}, count: {len(set(sd_keys_hf) - set(sd_keys))}"
        for k in sd_keys_hf:
...
print("I am GPU", ddp_rank, "of", ddp_world_size, "on device", device)
...
toekns = torch.tensor(tokens, dtype = torch.long)
tokens = toekns.unsqueeze(0).repeat(num_return_sequences, 1)
x = tokens.to(device)
#--------------------------------------------
...
while x.size(1) < max_lenght:
    ...
    topk_probs, topk_indices = torch.topk(probs, 50, dim=-1)  # get top 5 tokens
    ix = torch.multinomial(topk_probs, num_samples=1)  # sample from the top 5 tokens
